...
function logFailure(stage, batchId, attempt, error, timeoutSec) {
  ...
  const entry = JSON.stringify({
    ...
    error_type: error?.includes("ETIMEDOUT") ? "spawn_timeout"
      : error?.includes("rate") ? "rate_limited"
      : error?.includes("ECONNREFUSED") ? "connection_refused"
      : error?.includes("429") ? "api_throttle"
      : error?.includes("ENOMEM") ? "oom"
      : "unknown"
  }) + "\n";
  appendFileSync(join(runsDir, `${date}.jsonl`), entry);
}

function logSuccess(stage, batchId, attempt) {
  if (attempt <= 1) return;  // Don't log first-attempt successes (too noisy)
  const runsDir = join(ROOT, "runs");
  mkdirSync(runsDir, { recursive: true });
  const date = new Date().toISOString().split("T")[0];
  appendFileSync(join(runsDir, `${date}.jsonl`),
    JSON.stringify({
      ts: new Date().toISOString(), pipeline: "forge", agent: "pi-software",
      stage, batch: batchId, attempt, succeeded_after_retries: true, exit: 0
    }) + "\n");
}

// ─── Run pi stage with retry + progressive timeouts ───────────────
const MAX_RETRIES = 3;
const RETRY_BASE_DELAY_MS = 5000;  // 5s, doubles each retry
const PROGRESSIVE_TIMEOUT_FACTOR = 1.5;  // Each retry gets 1.5× more time

function classifyError(msg) {
  if (!msg) return "unknown";
  if (msg.includes("ETIMEDOUT")) return "spawn_timeout";
  if (msg.includes("rate") || msg.includes("429")) return "rate_limited";
  if (msg.includes("ECONNREFUSED")) return "connection_refused";
  if (msg.includes("ENOMEM")) return "oom";
  if (msg.includes("ENOTFOUND")) return "dns_failure";
  return "unknown";
}

function runPiStage(stageName, tools, prompt, sessionId, cwd, timeoutSec = 300, isFirst = false, batchId = "unknown") {
  const model = process.env.FORGE_PI_MODEL || "glm-5";
  const cmd = `${PI_BIN} --print --mode text --provider glm --model ${model} --thinking low --session-id ${sessionId} --tools ${tools} ${prompt}`;

  let lastError = null;

  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    const currentTimeout = Math.round(timeoutSec * Math.pow(PROGRESSIVE_TIMEOUT_FACTOR, attempt - 1));
    console.log(`  → ${stageName}${isFirst && attempt === 1 ? " (new session)" : " (continuing)"} [attempt ${attempt}/${MAX_RETRIES}, timeout ${currentTimeout}s]...`);
    ...
    try {
      ...
      logSuccess(stageName, batchId, attempt);
      if (attempt > 1) {
        console.log(`  ✅ ${stageName} succeeded on attempt ${attempt} (${currentTimeout}s timeout)`);
      }
      return { ok: true, output: result };

    } catch (e) {
      lastError = e.message;
      logFailure(stageName, batchId, attempt, e.message, currentTimeout);

      const errType = classifyError(e.message);
      if (attempt < MAX_RETRIES) {
        const delayMs = RETRY_BASE_DELAY_MS * Math.pow(2, attempt - 1);  // 5s, 10s, 20s
        const nextTimeout = Math.round(currentTimeout * PROGRESSIVE_TIMEOUT_FACTOR);
        console.log(`  ⚠️ ${stageName} attempt ${attempt} failed [${errType}]: ${e.message.substring(0, 200)}`);
        console.log(`  ↻ Retrying in ${delayMs / 1000}s with ${nextTimeout}s timeout...`);
        execSync(`sleep ${delayMs / 1000}`, { timeout: delayMs + 5000 });
      }
    }
  }

  console.error(`  ✗ ${stageName} failed after ${MAX_RETRIES} attempts. Last error: ${lastError?.substring(0, 300)}`);
...
async function runBatch(batch, plan, progress) {
  ...
  try {
    // ─── Stage 1: SPEC (new session, 180s base) ─────────
    ...
    const planResult = runPiStage("PLAN", "read,grep,ls,write",
      ...
      sessionId, WORKDIR, 180, false, batch.id);
    ...
    const buildResult = runPiStage("BUILD", "read,bash,edit,write",
      buildPrompt, sessionId, WORKDIR, 420, false, batch.id);
    if (!buildResult.ok) {
    ...
    const validateResult = runPiStage("VALIDATE", "read,bash,grep,ls,write",
      validatePrompt, sessionId, WORKDIR, 240, false, batch.id);
...
async function main() {
  ...
  console.log(`   Retry: ${MAX_RETRIES} attempts, progressive timeout ×${PROGRESSIVE_TIMEOUT_FACTOR}`);

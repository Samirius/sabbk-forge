...
// ─── Run pi stage ─────────────────────────────────────────────────
function runPiStage(stageName, tools, prompt, sessionId, cwd, timeoutSec = 180) {
  const cmd = `${PI_BIN} --print --mode text --provider glm --model glm-5.1 --thinking low --session-id ${sessionId} --tools ${tools} ${prompt}`;

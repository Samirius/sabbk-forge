...
function runPiStage(stageName, tools, prompt, sessionId, cwd, timeoutSec = 180, isFirst = false) {
  // First stage: --session-id to create the session
  // Later stages: --session-id + --continue to resume it
  const continueFlag = isFirst ? "" : "--continue";
  const cmd = `${PI_BIN} --print --mode text --provider glm --model glm-5.1 --thinking low ${continueFlag} --session-id ${sessionId} --tools ${tools} ${prompt}`;

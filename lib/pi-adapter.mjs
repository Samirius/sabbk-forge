...
export function buildArgs(a, stage, message, { resume = false } = {}) {
  ...
  // Per-stage model override (cost tiering): a stage may run on a cheaper model than the agent default
  // (e.g. smart sonnet for spec/plan judgment, cheap haiku for build/validate grunt).
  const sm = (a.model.stages && a.model.stages[stage]) || {};
  // Env overrides (PI_PROVIDER/PI_MODEL_ID) win over everything — used to run the whole agent on e.g. GLM.
  const provider = process.env.PI_PROVIDER || sm.provider || a.model.provider;
  const modelId = process.env.PI_MODEL_ID || sm.id || a.model.id;
  const thinking = sm.thinking || a.model.thinking || "low";
...
function main() {
  ...
  const timeoutMs = (a.budget && a.budget.timeout_sec ? a.budget.timeout_sec : 0) * 1000;  // per-stage wall-clock cap from the manifest
  const heapMb = parseInt(process.env.PI_NODE_HEAP_MB || "2048", 10);                       // bound pi's heap to reduce OOM on small VMs
  const childEnv = { ...process.env, NODE_OPTIONS: `--max-old-space-size=${heapMb} ${process.env.NODE_OPTIONS || ""}`.trim() };
  const child = spawn(bin, argv, { stdio: "inherit", cwd: join(ROOT, "spike", "workdir", a.id), env: childEnv, timeout: timeoutMs || undefined, killSignal: "SIGTERM" });

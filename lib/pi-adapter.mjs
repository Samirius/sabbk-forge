...
export function buildArgs(a, stage, message, { resume = false } = {}) {
  ...
  const argv = [
    ...
    "--session-id", `${a.id}-spike`,             // one session across stages so context carries
    "--tools", tools.join(","),                  // least-privilege allowlist for THIS stage
  ];
  if (resume) argv.push("--resume");
  if (resume) argv.splice(argv.indexOf("--session-id"), 2); // pi doesn't allow --session-id with --resume

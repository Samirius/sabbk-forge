...
function checkManifest(a) {
  ...
  if (!["coding", "domain", "orchestrator"].includes(a.kind)) fail(id, `kind must be coding|domain|orchestrator`);
  ...
  if (!a.budget || typeof a.budget.max_usd !== "number") fail(id, `budget.max_usd (number) required — the cost ceiling`);
...
// pipeline mode: every consecutive gear pair must be wired — the upstream gear must `produce`
// a handoff TO the downstream gear, and the downstream gear must `consume` a handoff FROM it.
function checkPipeline(agentsList) {
  const byId = Object.fromEntries(agentsList.map((a) => [a.id, a]));
  const dir = join(HERE, "..", "pipeline");
  let pfiles = [];
  try { pfiles = readdirSync(dir).filter((f) => f.endsWith(".json")); }
  catch { fail("pipeline", "no pipeline/ directory found"); return; }
  if (!pfiles.length) fail("pipeline", "no pipeline/*.json definitions found");
  for (const f of pfiles) {
    let def;
    try { def = JSON.parse(readFileSync(join(dir, f), "utf8")); }
    catch (e) { fail(f, `parse error: ${e.message}`); continue; }
    const steps = def.steps || [];
    if (steps.length < 2) { fail(f, "a pipeline needs >= 2 steps"); continue; }
    for (const s of steps) if (!byId[s.agent]) fail(f, `references unknown agent "${s.agent}"`);
    for (let i = 0; i < steps.length - 1; i++) {
      const A = byId[steps[i].agent], B = byId[steps[i + 1].agent];
      if (!A || !B) continue;
      const prod = (A.gear?.produces || []).find((p) => p.type === "handoff" && p.to === B.id);
      const cons = (B.gear?.consumes || []).find((c) => c.type === "handoff" && c.from === A.id);
      if (!prod) fail(f, `step ${i}->${i + 1}: ${A.id} does not produce a handoff to ${B.id}`);
      if (!cons) fail(f, `step ${i}->${i + 1}: ${B.id} does not consume a handoff from ${A.id}`);
    }
  }
}

if (mode === "pipeline") checkPipeline(agents);
else for (const a of agents) (mode === "gear" ? checkGear : checkManifest)(a);
...
console.log(`✓ ${mode} validation passed${mode === "pipeline" ? "" : ` for ${agents.length} agent(s)`}.`);

...
import { readFileSync, writeFileSync, mkdirSync, existsSync, unlinkSync, renameSync, statSync, rmdirSync, execSync } from "node:fs";
...
const lock = (dir) => {
  ...
  while (Date.now() - start < maxWait) {
    try {
      ...
      try {
        // B005 FIX: Read timestamp FROM the file, not filesystem mtime
        const ownerContent = readFileSync(ownerFile, "utf8").trim().split("\n");
        const lockTimestamp = parseInt(ownerContent[1] || "0", 10);
        const age = Date.now() - lockTimestamp;
        if (age > LOCK_MAX_AGE_MS) {
          console.warn(`  ⚠ removing stale budget lock (age ${Math.round(age / 1000)}s)`);
          forceRemoveLock(lf);
          continue;
        }
      } catch {}
      // B010 FIX: Synchronous sleep using exec (avoids Atomics.wait crash)
      execSync(`sleep 0.1`);
    }
  }

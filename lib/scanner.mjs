...
function detectStack() {
  ...
  // Extension-based fallback: detect languages from file extensions even without config files
  const extLangs = { ".php": "PHP", ".py": "Python", ".rb": "Ruby", ".go": "Go", ".rs": "Rust", ".java": "Java", ".ts": "TypeScript", ".tsx": "TypeScript", ".vue": "Vue SFC", ".svelte": "Svelte" };
  const seenExts = new Set();
  try {
    for (const f of allFiles || walkFiles(ABS)) {
      const ext = f.substring(f.lastIndexOf("."));
      if (extLangs[ext] && !stack.languages.includes(extLangs[ext])) {
        stack.languages.push(extLangs[ext]);
      }
    }
  } catch {}
...
const model = process.env.FORGE_SCANNER_MODEL || 'glm-4.6';
...
for (const [name, info] of modules) {
  ...
  modNum++;
  // Brief pause between modules to let V8 GC and avoid rate limits

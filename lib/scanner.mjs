...
Be concise. Be specific. Cite actual file names and line numbers you can see.`;
  ...
  const user = `Analyze this module:\n\n${modCode.slice(0, 8000)}`;

  try {
    const body = JSON.stringify({
      model: process.env.FORGE_SCANNER_MODEL || "glm-5",
    ...
    const resp = execSync(
      ...
      { input: body, timeout: 60000, encoding: "utf8", maxBuffer: 5 * 1024 * 1024 }
    );
    const parsed = JSON.parse(resp);
    return parsed.choices?.[0]?.message?.content || "(empty response)";
  } catch (e) {
    return `## Analysis\n(LLM call failed: ${e.message}. Raw code preserved below.)`;
  }
}

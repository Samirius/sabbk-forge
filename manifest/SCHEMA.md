...
| `kind` | string | yes | `coding` (uses pi-coding-agent) or `domain` (non-coding Pi agent). |
...
| `budget.max_turns` | number | yes | Hard cap on agent-loop turns. The #1 run-cost control. |
| `budget.max_usd` | number | yes | Soft budget ceiling surfaced to the operator. |
